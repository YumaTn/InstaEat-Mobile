...
export default function Review({ route, navigation }) {
  ...
  const submitReview = async () => {
    try {
      ...
      console.log('Error Details:', error); // In ra chi tiết lỗi
      if (error.response) {
  ...
  return (
    <View style={styles.container}>
      ...
      />
      <TouchableOpacity style={styles.UploadImage} onPress={pickImage} ><AntDesign name="picture" size={24} color="black" />
      <Text style={styles.UploadImageText}>Tải ảnh lên</Text>
      </TouchableOpacity>
      {imageUri && <Image source={{ uri: imageUri }} style={styles.image} />}
      <TouchableOpacity style={styles.button} onPress={submitReview}><Text style={styles.submit}>Xác nhận</Text></TouchableOpacity>
    </View>
  );
}
